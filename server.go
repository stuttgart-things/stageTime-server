...
func (s Server) CreateRevisionRun(ctx context.Context, gRPCRequest *revisionrun.CreateRevisionRunRequest) (*revisionrun.Response, error) {
	...
	for i := 0; i < (len(renderedPipelineruns)); i++ {
		...
		for _, pr := range renderedPipelineruns[i] {
			...
			log.Info("IDENTIFIER", prIdentifier)
			log.Info("REVISIONRUN-ID", revisionRunID)
			...
			log.Info("PIPELINERUN WAS ADDED TO REDIS (JSON): ", resourceName)
			...
			sthingsCli.DeleteRedisSet(redisClient, revisionRunID+"-"+stage)
			sthingsCli.AddValueToRedisSet(redisClient, revisionRunID+"-"+stage, resourceName)
			log.Info("ADDED PIPELINERUN NAMES TO REDIS (SET): ", revisionRunID+"-"+stage)
		}
	}
	...
	for key, index := range stages {
		...
		initialStageStatus := server.StageStatus{
			StageID:           revisionRunID + "-" + sthingsBase.ConvertIntegerToString(stageNumber),
			CountPipelineRuns: stages[key],
			LastUpdated:       now.Format("2006-01-02 15:04:05"),
			Status:            "CREATED W/ STAGETIME-SERVER",
		}
		sthingsCli.SetRedisJSON(redisJSONHandler, initialStageStatus, revisionRunID+"-"+sthingsBase.ConvertIntegerToString(stageNumber))
		log.Info("INITIAL STATE STATUS WAS ADDED TO REDIS (JSON): ", revisionRunID+"-"+sthingsBase.ConvertIntegerToString(stageNumber))
